...
def parse_betclic_api(id_league,live=False): #<=== add live=false
    """
...
def parse_betclic(url,live=False): #<==== add live=False
    """
    Get odds from Betclic url
    """
    if "-s" in url and url.split("-s")[-1].isdigit():
        # print("url split :",url.split("-s")[-1])
        return parse_sport_betclic(url.split("-s")[-1],live) #<=== add live
    id_league = re.findall(r'\d+', url)[-1]
    return parse_betclic_api(id_league,live) #<== add live


def parse_sport_betclic(id_sport,live=False): #<==== add live=False
    """
    ...
    for competition in competitions:
        ...
        # print("comeptition : ",competition)
        # print("id : ",id_competition)
        list_odds.append(parse_betclic_api(id_competition,live))#<=== add live
